...
function oc_scale_one
  oc get dc -o json | python -c "import json, sys, os; l=[o['metadata']['name'] for o in json.load(sys.stdin)['items'] if o['spec']['replicas'] != 1]; [os.system('oc scale dc %s --replicas=1' % o) for o in l]"
end

function oc_scale_zero
  oc get dc -o json | python -c "import json, sys, os; l=[o['metadata']['name'] for o in json.load(sys.stdin)['items'] if o['spec']['replicas'] != 0]; [os.system('oc scale dc %s --replicas=0' % o) for o in l]"
end